...
/**
 * Recruiter & Contractor Dashboard with interactive card hover effects and
...
public class RecruiterDashboard {
    ...
    public RecruiterDashboard(Recruiter recruiter) {
        this.recruiter = recruiter != null ? recruiter
    ...
    private HBox createHeader(Recruiter currentR, String name, String company, Runnable back) {
        ...
        Label logo = label("DIHADI", "-fx-font-family:Georgia;-fx-font-size:26px;-fx-font-weight:800;-fx-text-fill:"
                + GOLD + ";-fx-letter-spacing:1px;");
        ...
        logoutBtn.setOnAction(e -> {
            ...
            NotificationToast.show("Signed Out", "You have signed out of your recruiter session.",
        ...
        HBox accountActions = new HBox(10, backBtn, profileBadge, logoutBtn);
    ...
    private HBox createHero(Recruiter currentR, String name, String company, Runnable back) {
        ...
        Label subtextLabel = label(
                "Manage your active project sites, assigned workforce, candidate worker approvals, and wallet balance from one place.",
        ...
        activeOngoingPanel.setStyle("-fx-background-color:#ffffff;-fx-background-radius:14px;-fx-border-color:" + BORDER
                + ";-fx-border-width:1.5px;-fx-border-radius:14px;-fx-effect:dropshadow(gaussian,rgba(58,48,39,.05),10,0,0,2px);");
        activeOngoingPanel.setOnMouseEntered(e -> activeOngoingPanel.setStyle(
                "-fx-background-color:#ffffff;-fx-background-radius:14px;-fx-border-color:#d4af37;-fx-border-width:2px;-fx-border-radius:14px;-fx-effect:dropshadow(gaussian,rgba(212,175,55,.25),14,0,0,4px);-fx-cursor:hand;"));
        activeOngoingPanel.setOnMouseExited(e -> activeOngoingPanel
                .setStyle("-fx-background-color:#ffffff;-fx-background-radius:14px;-fx-border-color:" + BORDER
                        + ";-fx-border-width:1.5px;-fx-border-radius:14px;-fx-effect:dropshadow(gaussian,rgba(58,48,39,.05),10,0,0,2px);"));
        ...
        heroRow.setStyle("-fx-background-color:#ffffff;-fx-background-radius:16px;-fx-border-color:" + BORDER
                + ";-fx-border-width:1.5px;-fx-border-radius:16px;-fx-effect:dropshadow(gaussian,rgba(58,48,39,.06),12,0,0,3px);");
        return heroRow;
    }
    ...
    private HBox createMetrics(Recruiter currentR, Runnable back) {
        ...
        VBox walletMetric = new VBox(6, walletHeadRow, walletBalanceLabel,
                label("Available deposit balance", "-fx-font-size:12px;-fx-font-weight:700;-fx-text-fill:#2e7d32;"));
        walletMetric.setPadding(new Insets(16, 18, 16, 18));
        walletMetric.setPrefHeight(115);
        walletMetric.setStyle("-fx-background-color:#ffffff;-fx-background-radius:14px;-fx-border-color:" + BORDER
                + ";-fx-border-width:1.5px;-fx-border-radius:14px;-fx-effect:dropshadow(gaussian,rgba(58,48,39,.05),8,0,0,2px);");
        walletMetric.setOnMouseEntered(e -> walletMetric.setStyle(
                "-fx-background-color:#ffffff;-fx-background-radius:14px;-fx-border-color:#d4af37;-fx-border-width:2px;-fx-border-radius:14px;-fx-effect:dropshadow(gaussian,rgba(212,175,55,.25),14,0,0,4px);-fx-cursor:hand;"));
        walletMetric.setOnMouseExited(e -> walletMetric
                .setStyle("-fx-background-color:#ffffff;-fx-background-radius:14px;-fx-border-color:" + BORDER
                        + ";-fx-border-width:1.5px;-fx-border-radius:14px;-fx-effect:dropshadow(gaussian,rgba(58,48,39,.05),8,0,0,2px);"));

        workersMetricLabel = label("0",
                "-fx-font-family:Georgia;-fx-font-size:26px;-fx-font-weight:800;-fx-text-fill:#1e1b15;");
        workersMetricSub = label("Workers on active project",
        ...
        projectsMetricLabel = label("0",
                "-fx-font-family:Georgia;-fx-font-size:26px;-fx-font-weight:800;-fx-text-fill:#1565c0;");
        VBox projectsMetric = kpiCard("TOTAL PROJECTS", projectsMetricLabel,
                label("Created projects", "-fx-font-size:12px;-fx-font-weight:700;-fx-text-fill:#1565c0;"));

        reqCountLabel = label("0",
                "-fx-font-family:Georgia;-fx-font-size:26px;-fx-font-weight:800;-fx-text-fill:#ba1a1a;");
        VBox approvalsMetric = kpiCard("PENDING APPROVALS", reqCountLabel,
                label("Worker applications", "-fx-font-size:12px;-fx-font-weight:700;-fx-text-fill:#ba1a1a;"));
    ...
    private VBox kpiCard(String title, Label numberNode, Label subtextNode) {
        ...
        card.setOnMouseExited(
                e -> card.setStyle("-fx-background-color:#ffffff;-fx-background-radius:14px;-fx-border-color:" + BORDER
                        + ";-fx-border-width:1.5px;-fx-border-radius:14px;-fx-effect:dropshadow(gaussian,rgba(58,48,39,.05),8,0,0,2px);"));
        return card;
    }
    ...
    private HBox createBody(Recruiter currentR, Runnable back) {
        ...
        reqPanel = executivePanel("Pending Worker Applications",
                label("Loading candidates...", "-fx-font-size:13px;-fx-text-fill:#685c52;"));
        notificationsPanel = executivePanel("Notifications",
                label("Loading updates...", "-fx-font-size:13px;-fx-text-fill:#685c52;"));
    ...
    private void pollRecruiterLiveState(Recruiter currentR, Runnable back) {
        ...
        new Thread(() -> {
            try {
                String fullName = (val(currentR.getFirstName(), "Recruiter") + " " + val(currentR.getLastName(), ""))
                        .trim();
                String company = val(currentR.getCompanyName(), "Construction Group");
                ...
                List<Notification> recruiterNotifs = new NotificationController()
                        .getNotifications(currentR.getMobileNumber());
                ...
                if (allProjects != null) {
                    List<Project> sortedAll = new ArrayList<>(allProjects);
                ...
                String rMobDigits = currentR.getMobileNumber() != null
                        ? currentR.getMobileNumber().replaceAll("\\D", "")
                        : "";

                Map<String, JobApplication> deduplicatedPending = new java.util.LinkedHashMap<>();
                Map<String, JobApplication> sentHiringOffers = new java.util.LinkedHashMap<>();
                if (allApps != null) {
                    for (JobApplication a : allApps) {
                        if ("Pending".equalsIgnoreCase(a.getStatus())) {
                            boolean isOutgoingHire = (a.getJobTitle() != null
                                    && a.getJobTitle().contains("Hiring Request"))
                                    || "DIRECT_HIRE".equalsIgnoreCase(a.getRequirementId());

                            boolean matchesRecruiter = false;
                            if (a.getProjectId() != null && recruiterProjIds.contains(a.getProjectId())) {
                                matchesRecruiter = true;
                            } else {
                                String appRMob = a.getRecruiterMobile() != null
                                        ? a.getRecruiterMobile().replaceAll("\\D", "")
                                        : "";
                                if (!rMobDigits.isEmpty() && !appRMob.isEmpty() && (appRMob.equals(rMobDigits)
                                        || appRMob.endsWith(rMobDigits) || rMobDigits.endsWith(appRMob))) {
                                    matchesRecruiter = true;
                                }
                            }

                            if (matchesRecruiter) {
                                String key = (a.getWorkerMobile() != null ? a.getWorkerMobile().replaceAll("\\D", "")
                                        : "") + "_" + (a.getProjectId() != null ? a.getProjectId() : "");
                                if (isOutgoingHire) {
                                    sentHiringOffers.putIfAbsent(key, a);
                                } else {
                ...
                final List<JobApplication> finalSentOffersList = new ArrayList<>(sentHiringOffers.values());
                ...
                Platform.runLater(() -> {
                    ...
                    if (workersMetricSub != null) {
                        workersMetricSub
                                .setText(finalActiveProj != null ? "Assigned to " + finalActiveProj.getProjectName()
                                        : "No active project");
                    }
                    ...
                    if (activeOngoingPanel != null) {
                        ...
                        if (finalActiveProj == null) {
                            Label noProj = label("No Active Project Running",
                                    "-fx-font-family:Georgia;-fx-font-size:18px;-fx-font-weight:800;-fx-text-fill:#1e1b15;");
                            ...
                            createProjBtn.setStyle(
                                    "-fx-background-color:#272727;-fx-background-radius:8px;-fx-text-fill:#ffd54f;-fx-font-size:11px;-fx-font-weight:800;-fx-padding:7px 16px;-fx-cursor:hand;");
                            createProjBtn.setOnAction(ev -> {
                                Stage stage = (Stage) createProjBtn.getScene().getWindow();
                                stage.setScene(new CreateProjectPage()
                                        .getCreateProjectScene(() -> stage.setScene(getScene(back))));
                            });
                            ...
                            Label projName = label(finalActiveProj.getProjectName(),
                                    "-fx-font-family:Georgia;-fx-font-size:19px;-fx-font-weight:800;-fx-text-fill:#1e1b15;");
                            VBox contactDetail = detail("Supervisor", val(finalActiveProj.getContactName(), fullName));
                            String loc = (val(finalActiveProj.getCity(), "") + ", "
                                    + val(finalActiveProj.getState(), "")).replaceAll("^, |, $", "");
                            ...
                            completeBtn.setStyle(
                                    "-fx-background-color:#e8f5e9;-fx-background-radius:8px;-fx-text-fill:#1b5e20;-fx-border-color:#c8e6c9;-fx-border-radius:8px;-fx-font-size:11px;-fx-font-weight:800;-fx-padding:7px 16px;-fx-cursor:hand;");
                            completeBtn.setOnAction(ev -> {
                                ...
                                new Thread(() -> {
                                    ...
                                    if (finalActiveProj.getProjectId() != null) {
                                        new com.dihadi.dao.JobApplicationDao().updateProjectApplicationsStatus(
                                                finalActiveProj.getProjectId(), "Completed");
                                    ...
                                    Platform.runLater(() -> {
                                        ...
                                        com.dihadi.view.NotificationToast.show(stage, "Project Completed",
                                                finalActiveProj.getProjectName()
                                                        + " has been completed, moved to Past Projects, and removed from worker browsing.",
                            ...
                            Button addWorkforceHeroBtn = new Button("+ Add More Workforce");
                            addWorkforceHeroBtn.setStyle(
                                    "-fx-background-color:#272727;-fx-background-radius:8px;-fx-text-fill:#ffd54f;-fx-font-size:11px;-fx-font-weight:800;-fx-padding:7px 16px;-fx-cursor:hand;");
                            addWorkforceHeroBtn.setOnAction(ev -> {
                                if (livePoller != null) livePoller.stop();
                                Stage stage = (Stage) addWorkforceHeroBtn.getScene().getWindow();
                                String pImg = (finalActiveProj.getImageUrls() != null && !finalActiveProj.getImageUrls().isEmpty())
                                        ? finalActiveProj.getImageUrls().get(0) : "";
                                String pAddr = finalActiveProj.getAddressLine1() != null ? finalActiveProj.getAddressLine1()
                                        : (finalActiveProj.getCity() + ", " + finalActiveProj.getState());
                                stage.setScene(new AddWorkersPage(finalActiveProj.getProjectId(), finalActiveProj.getProjectName(),
                                        val(finalActiveProj.getContactName(), fullName), finalActiveProj.getMobile(), finalActiveProj.getEmail(),
                                        pAddr, pImg).getAddWorkersScene(() -> stage.setScene(getScene(back))));
                            });
                            foot.getChildren().add(addWorkforceHeroBtn);

                            if (!isFulfilled) {
                                Button fulfillBtn = new Button("Mark Requirement Fulfilled");
                                fulfillBtn.setStyle(
                                        "-fx-background-color:#fff8e1;-fx-background-radius:8px;-fx-text-fill:#b48700;-fx-border-color:#ffe082;-fx-border-radius:8px;-fx-font-size:11px;-fx-font-weight:800;-fx-padding:7px 16px;-fx-cursor:hand;");
                                fulfillBtn.setOnAction(ev -> {
                                    fulfillBtn.setDisable(true);
                                    new Thread(() -> {
                                        finalActiveProj.setStatus("Requirement Fulfilled");
                                        new ProjectController().addProject(finalActiveProj);
                                        Platform.runLater(() -> {
                                            Stage stage = (Stage) fulfillBtn.getScene().getWindow();
                                            com.dihadi.view.NotificationToast.show(stage, "Requirement Fulfilled",
                                                    finalActiveProj.getProjectName()
                                                            + " status is now 'Requirement Fulfilled' in the worker interface.",
                                                    com.dihadi.view.NotificationToast.ToastType.SUCCESS);
                                            stage.setScene(getScene(back));
                                        });
                                    }).start();
                                });
                                foot.getChildren().add(fulfillBtn);
                            }
                            foot.getChildren().add(completeBtn);
                    ...
                    if (activeProjPanel != null) {
                        ...
                        if (activeList.isEmpty()) {
                            ...
                            for (Project actP : activeList) {
                                String locStr = (val(actP.getCity(), "") + ", " + val(actP.getState(), ""))
                                        .replaceAll("^, |, $", "");
                                ...
                                VBox activeCard = new VBox(6,
                                        label(actP.getProjectName(),
                                                "-fx-font-size:14px;-fx-font-weight:800;-fx-text-fill:#1e1b15;"),
                                        label("Location: " + (locStr.isBlank() ? "Pune" : locStr),
                                                "-fx-font-size:12px;-fx-text-fill:#4d4635;"),
                                        stLabel);

                                HBox actBtnRow = new HBox(8);
                                actBtnRow.setAlignment(Pos.CENTER_LEFT);

                                Button addWorkforceCardBtn = new Button("+ Add More Workforce");
                                addWorkforceCardBtn.setStyle(
                                        "-fx-background-color:#272727;-fx-background-radius:6px;-fx-text-fill:#ffd54f;-fx-font-size:10px;-fx-font-weight:800;-fx-padding:4px 10px;-fx-cursor:hand;");
                                addWorkforceCardBtn.setOnAction(e -> {
                                    if (livePoller != null) livePoller.stop();
                                    Stage stage = (Stage) addWorkforceCardBtn.getScene().getWindow();
                                    String pImg = (actP.getImageUrls() != null && !actP.getImageUrls().isEmpty())
                                            ? actP.getImageUrls().get(0) : "";
                                    String pAddr = actP.getAddressLine1() != null ? actP.getAddressLine1()
                                            : (actP.getCity() + ", " + actP.getState());
                                    stage.setScene(new AddWorkersPage(actP.getProjectId(), actP.getProjectName(),
                                            val(actP.getContactName(), fullName), actP.getMobile(), actP.getEmail(),
                                            pAddr, pImg).getAddWorkersScene(() -> stage.setScene(getScene(back))));
                                });
                                actBtnRow.getChildren().add(addWorkforceCardBtn);

                                if (!isActFulfilled) {
                                    Button fulfillProjectBtn = new Button("Mark Fulfilled");
                                    fulfillProjectBtn.setStyle(
                                            "-fx-background-color:#fff8e1;-fx-background-radius:6px;-fx-text-fill:#b48700;-fx-border-color:#ffe082;-fx-border-radius:6px;-fx-font-size:10px;-fx-font-weight:800;-fx-padding:4px 10px;-fx-cursor:hand;");
                                    fulfillProjectBtn.setOnAction(e -> {
                                        fulfillProjectBtn.setDisable(true);
                                        new Thread(() -> {
                                            actP.setStatus("Requirement Fulfilled");
                                            new ProjectController().addProject(actP);
                                            Platform.runLater(() -> {
                                                Stage stage = (Stage) fulfillProjectBtn.getScene().getWindow();
                                                com.dihadi.view.NotificationToast.show(stage, "Requirement Fulfilled",
                                                        actP.getProjectName() + " is now marked as 'Requirement Fulfilled'.",
                                                        com.dihadi.view.NotificationToast.ToastType.SUCCESS);
                                                stage.setScene(getScene(back));
                                            });
                                        }).start();
                                    });
                                    actBtnRow.getChildren().add(fulfillProjectBtn);
                                }

                                Button releaseBtn = new Button("Complete Project");
                                releaseBtn.setStyle(
                                        "-fx-background-color:#e8f5e9;-fx-background-radius:6px;-fx-text-fill:#1b5e20;-fx-border-color:#c8e6c9;-fx-border-radius:6px;-fx-font-size:10px;-fx-font-weight:800;-fx-padding:4px 10px;-fx-cursor:hand;");
                                releaseBtn.setOnAction(e -> {
                                    releaseBtn.setDisable(true);
                                    new Thread(() -> {
                                        actP.setStatus("Completed");
                                        new ProjectController().addProject(actP);
                                        if (actP.getProjectId() != null) {
                                            new com.dihadi.dao.JobApplicationDao().updateProjectApplicationsStatus(
                                        ...
                                        Platform.runLater(() -> {
                                            Stage stage = (Stage) releaseBtn.getScene().getWindow();
                                            com.dihadi.view.NotificationToast.show(stage, "Project Completed",
                                                    actP.getProjectName()
                                                            + " is now completed, moved to Past Projects, and removed from worker browsing.",
                                                    com.dihadi.view.NotificationToast.ToastType.SUCCESS);
                                            stage.setScene(getScene(back));
                                        });
                                    }).start();
                                });
                                actBtnRow.getChildren().add(releaseBtn);
                                activeCard.getChildren().add(actBtnRow);
                                ...
                                activeCard.setOnMouseEntered(e -> activeCard.setStyle(
                    ...
                    if (pastProjPanel != null) {
                        ...
                        if (finalCompleted.isEmpty()) {
                            pastProjPanel.getChildren().add(label("No completed project records yet.",
                                    "-fx-font-size:13px;-fx-text-fill:#685c52;"));
                        } else {
                            for (Project cp : finalCompleted) {
                                String cpLoc = (val(cp.getCity(), "") + ", " + val(cp.getState(), ""))
                                        .replaceAll("^, |, $", "");
                                VBox cpCard = new VBox(4,
                                        label(cp.getProjectName(),
                                                "-fx-font-size:14px;-fx-font-weight:800;-fx-text-fill:#1e1b15;"),
                                        label("Location: " + (cpLoc.isBlank() ? "Pune" : cpLoc),
                                                "-fx-font-size:12px;-fx-text-fill:#4d4635;"),
                                ...
                                cpCard.setOnMouseEntered(e -> cpCard.setStyle(
                                        "-fx-background-color:#ffffff;-fx-background-radius:8px;-fx-border-color:#d4af37;-fx-border-width:1.5px;-fx-border-radius:8px;-fx-cursor:hand;"));
                                cpCard.setOnMouseExited(e -> cpCard.setStyle(
                                        "-fx-background-color:#f4f9f4;-fx-background-radius:8px;-fx-border-color:#c8e6c9;-fx-border-width:1px;-fx-border-radius:8px;"));
                    ...
                    if (reqPanel != null) {
                        ...
                        if (finalPendingList.isEmpty()) {
                            reqPanel.getChildren().add(label("No pending worker applications awaiting action.",
                                    "-fx-font-size:13px;-fx-text-fill:#685c52;"));
                        } else {
                            for (JobApplication a : finalPendingList) {
                                Label nameLbl = label(
                                        "Applicant Worker (Mobile: " + val(a.getWorkerMobile(), "Worker") + ")",
                                        "-fx-font-size:13px;-fx-font-weight:800;-fx-text-fill:#1e1b15;");
                                Label jobLbl = label(
                                        "Position: " + val(a.getJobTitle(), "Site Position") + "  |  Daily Wage: Rs. "
                                                + val(a.getJobWage(), "950"),
                                        "-fx-font-size:12px;-fx-text-fill:#4d4635;");

                                Button viewAllBtn = new Button("Review Application");
                                viewAllBtn.setStyle(
                                        "-fx-background-color:#272727;-fx-background-radius:6px;-fx-text-fill:#ffd54f;-fx-font-size:11px;-fx-font-weight:800;-fx-padding:5px 12px;-fx-cursor:hand;");
                                viewAllBtn.setOnAction(ev -> {
                                    if (livePoller != null)
                                        livePoller.stop();
                                    Stage stage = (Stage) viewAllBtn.getScene().getWindow();
                                    stage.setScene(new PendingApprovalsPage(currentR)
                                            .getScene(() -> stage.setScene(getScene(back))));
                                });
                                ...
                                appCard.setStyle(
                                        "-fx-background-color:#faf5eb;-fx-background-radius:8px;-fx-border-color:#ebdccb;-fx-border-width:1px;-fx-border-radius:8px;");
                                appCard.setOnMouseEntered(e -> appCard.setStyle(
                                        "-fx-background-color:#ffffff;-fx-background-radius:8px;-fx-border-color:#d4af37;-fx-border-width:1.5px;-fx-border-radius:8px;-fx-cursor:hand;"));
                                appCard.setOnMouseExited(e -> appCard.setStyle(
                                        "-fx-background-color:#faf5eb;-fx-background-radius:8px;-fx-border-color:#ebdccb;-fx-border-width:1px;-fx-border-radius:8px;"));
                                reqPanel.getChildren().add(appCard);
                            }
                        }

                        if (!finalSentOffersList.isEmpty()) {
                            Label sentHeader = label("Sent Job Offers (Awaiting Worker Response)",
                                    "-fx-font-size:14px;-fx-font-weight:800;-fx-text-fill:#735c00;-fx-padding:14px 0 4px 0;");
                            reqPanel.getChildren().add(sentHeader);
                            for (JobApplication offer : finalSentOffersList) {
                                Label targetWorker = label(
                                        "Offered Worker (Mobile: " + val(offer.getWorkerMobile(), "Worker") + ")",
                                        "-fx-font-size:13px;-fx-font-weight:800;-fx-text-fill:#1e1b15;");
                                Label offerDetails = label(
                                        "Offer: " + val(offer.getJobTitle(), "Site Position") + "  |  Daily Wage: Rs. "
                                                + val(offer.getJobWage(), "Standard Rate"),
                                        "-fx-font-size:12px;-fx-text-fill:#4d4635;");
                                Label statusBadge = label("Status: Awaiting Worker Acceptance",
                                        "-fx-font-size:11px;-fx-font-weight:700;-fx-text-fill:#b48700;-fx-background-color:#fff8e1;-fx-background-radius:6px;-fx-padding:4px 8px;");

                                VBox offerCard = new VBox(5, targetWorker, offerDetails, statusBadge);
                                offerCard.setPadding(new Insets(10, 12, 10, 12));
                                offerCard.setStyle(
                                        "-fx-background-color:#fcfaf6;-fx-background-radius:8px;-fx-border-color:#ebdccb;-fx-border-width:1px;-fx-border-radius:8px;");
                                reqPanel.getChildren().add(offerCard);
                            }
                        }
                    }
                        ...
                        if (recruiterNotifs == null || recruiterNotifs.isEmpty()) {
                            notificationsPanel.getChildren().addAll(
                                    label("No notifications yet",
                                            "-fx-font-size:13px;-fx-font-weight:700;-fx-text-fill:#4c4637;"),
                                    label("Worker applications and accepted job offers will appear here.",
                                            "-fx-font-size:12px;-fx-text-fill:#8c7e6b;"));
                        } else {
                            int count = 0;
                            for (Notification n : recruiterNotifs) {
                                if (n.getNotificationId() != null
                                        && !seenRecruiterNotifIds.contains(n.getNotificationId())) {
                                ...
                                notifCard.setStyle(
                                        "-fx-background-color:#faf5eb;-fx-background-radius:8px;-fx-border-color:#ebdccb;-fx-border-width:1px;-fx-border-radius:8px;");
                                notifCard.setOnMouseEntered(e -> notifCard.setStyle(
                                        "-fx-background-color:#ffffff;-fx-background-radius:8px;-fx-border-color:#d4af37;-fx-border-width:1.5px;-fx-border-radius:8px;-fx-cursor:hand;"));
                                notifCard.setOnMouseExited(e -> notifCard.setStyle(
                                        "-fx-background-color:#faf5eb;-fx-background-radius:8px;-fx-border-color:#ebdccb;-fx-border-width:1px;-fx-border-radius:8px;"));
    ...
    private void displayAlert(Alert.AlertType type, String title, String message) {
        ...
        if (type == Alert.AlertType.ERROR)
            toastType = NotificationToast.ToastType.ERROR;
        else if (type == Alert.AlertType.WARNING)
            toastType = NotificationToast.ToastType.ALERT;
    ...
    private VBox executivePanel(String heading, Node... nodes) {
        ...
        panel.setStyle("-fx-background-color:#ffffff;-fx-background-radius:14px;-fx-border-color:" + BORDER
                + ";-fx-border-width:1.5px;-fx-border-radius:14px;-fx-effect:dropshadow(gaussian,rgba(58,48,39,.05),8,0,0,2px);");
        panel.setOnMouseEntered(e -> panel.setStyle(
                "-fx-background-color:#ffffff;-fx-background-radius:14px;-fx-border-color:#d4af37;-fx-border-width:2px;-fx-border-radius:14px;-fx-effect:dropshadow(gaussian,rgba(212,175,55,.20),14,0,0,4px);"));
        panel.setOnMouseExited(
                e -> panel.setStyle("-fx-background-color:#ffffff;-fx-background-radius:14px;-fx-border-color:" + BORDER
                        + ";-fx-border-width:1.5px;-fx-border-radius:14px;-fx-effect:dropshadow(gaussian,rgba(58,48,39,.05),8,0,0,2px);"));
        return panel;
    }
    ...
    private Label panelHeader(String text) {
        return label(text,
                "-fx-font-size:12px;-fx-font-weight:800;-fx-letter-spacing:0.8px;-fx-text-fill:" + PRIMARY + ";");
    }

    private Button nav(String text, boolean active) {
        Button b = new Button(text);
        b.setStyle("-fx-background-color:" + (active ? "#faf3e8" : "transparent") + ";-fx-text-fill:" + GOLD
                + ";-fx-font-size:13px;-fx-font-weight:800;-fx-padding:6px 14px;-fx-background-radius:8px;-fx-cursor:hand;");
        return b;
    }
    ...
    private boolean isMatch(Project p, Recruiter r, String searchKey) {
        ...
        if (!pMob.isEmpty() && !rMob.isEmpty() && (pMob.equals(rMob) || pMob.endsWith(rMob) || rMob.endsWith(pMob)))
            return true;
        if (!pMob.isEmpty() && !sKeyDigits.isEmpty()
                && (pMob.equals(sKeyDigits) || pMob.endsWith(sKeyDigits) || sKeyDigits.endsWith(pMob)))
            return true;
        if (!pEmail.isEmpty() && (!rEmail.isEmpty() && pEmail.equals(rEmail)))
            return true;
        if (!pEmail.isEmpty() && (!sKey.isEmpty() && pEmail.equals(sKey)))
            return true;
